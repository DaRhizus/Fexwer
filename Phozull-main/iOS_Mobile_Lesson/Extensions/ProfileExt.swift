...
extension ProfileVC: ProfileDelegate {
    ...
    func didUpdateProfile(success: Bool) {
        if success {
            let alert = UIAlertController(title: "Başarılı", message: "Profil başarıyla güncellendi.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "Tamam", style: .default, handler: nil))
            present(alert, animated: true, completion: nil)
        } else {
            let alert = UIAlertController(title: "Hata", message: "Profil güncelleme başarısız.", preferredStyle: .alert)
            alert.addAction(UIAlertAction(title: "Tamam", style: .default, handler: nil))
            present(alert, animated: true, completion: nil)
        }
    }
